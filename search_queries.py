...
def run_search(dict_file, postings_file, queries_file, results_file):
    ...
    path_to_query_file=queries_file
    ...
    if os.path.isdir(path_to_query_file):
        # List all files in the directory
        files = os.listdir(path_to_query_file)
        
        # Loop through each file
        for file_name in files:
            # Construct the full path to the file
            file_path = os.path.join(path_to_query_file, file_name)
            ...
            if os.path.isfile(file_path):
                ...
                with open(file_path, 'r') as file:
                    ...
                    query=lines[0]
                    if "AND" not in query:
                        handle_free_text_queries(lines[0],dictionary,number_of_docs,posting_list,dict_docids_to_length,file_name)
                    else: 
                        handle_boolean_queries(query,dictionary,posting_list,file_name,number_of_docs,dict_docids_to_length)
    else:
        print(f"{path_to_query_file} is not a directory.")

def handle_boolean_queries(query,dictionary,posting_list,file_name,number_of_docs,length_dictionary):
    ...
    if not all_docs:
        with open('output/'+file_name+'_output'+'.txt','w') as out_file:
            out_file.write(""+"\n")
    else:
...
def handle_free_text_queries(query,dictionary,number_of_docs,posting_list,length_dictionary,file_name):
